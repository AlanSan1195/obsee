import { getMicProfileFromGroq } from './_lib/groq';
...
export default async function handler(request: ApiRequest, response: ApiResponse) {
  ...
  try {
    const validation = validateMicProfileRequest(readBody(request));
    if (!validation.success) {
      return sendJson(response, 400, { message: validation.message });
    }

    const aiPayload = await getMicProfileFromGroq(validation.value);
    const profile = validateMicProfileResponse(aiPayload);
    if (!profile.success) {
    ...
    console.error('Audio profile endpoint failed:', error);
    return sendJson(response, 500, {
      message: 'La IA integrada no pudo analizar el microfono.',
    });
  }
}
